...
Since A is a real symmetric matrix there exists a real orthogonal matrix P such that 
 \begin{equation*}
     D=P^TAP
 \end{equation*}
is a diagonal matrix. Furthermore, the entries along the diagonal of D is the eigenvalues of A. Since the matrix product of two orthogonal matrices is another orthogonal matrix, we can preform a series of similarity transformations until we get a diagonal matrix. That is 
\begin{equation*}
S_k^TS_{k-1}^T\cdots S_1^TAS_1 \cdots S_{k-1}S_k = D
\end{equation*}
where $S_i, \quad i = 1, \ldots, k $ are real orthogonal matrices.  
