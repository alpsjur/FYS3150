...
\label{sec:jacobi}
The strategy of Jacobi's method is to preform a series of similarity transformations on the matrix A in order to diagonalize the matrix. We say that matrix B is similar to A if
 \begin{equation*}
	B = S^{-1}AS, 
 \end{equation*}
...
Since A is a real symmetric matrix there exists a real orthogonal matrix P such that 
 \begin{equation*}
     D=P^TAP
 \end{equation*}
is a diagonal matrix. Furthermore, the entries along the diagonal of D is the eigenvalues of A.  
